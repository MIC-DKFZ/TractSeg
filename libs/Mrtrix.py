...
class Mrtrix():
    ...
    def create_fods(input_file, output_dir, bvals, bvecs, csd_resolution):
        ...
        if csd_resolution == "HIGH":
            ...
            os.system("dwi2fod msmt_csd " + input_file + " RF_WM.txt WM_FODs.mif RF_GM.txt GM.mif RF_CSF.txt CSF.mif -mask nodif_brain_mask.nii.gz -fslgrad " + bvecs + " " + bvals)       # multi-shell, multi-tissue
            ...
            # os.system("dwi2response dhollander -mask nodif_brain_mask.nii.gz " + input_file + " RF_WM_DHol.txt RF_GM_DHol.txt RF_CSF_DHol.txt -fslgrad " + bvecs + " " + bvals)
            ...
            # os.system("dwi2fod msmt_csd " + input_file + " RF_WM_DHol.txt WM_FODs.mif -fslgrad " + bvecs + " " + bvals + " -mask nodif_brain_mask.nii.gz")

            os.system("sh2peaks WM_FODs.mif peaks.nii.gz")
        else:   #LOW
            # CSD Tournier
            os.system("dwi2response tournier " + input_file + " response.txt -mask nodif_brain_mask.nii.gz -fslgrad " + bvecs + " " + bvals + " -force")
            os.system("dwi2fod csd " + input_file + " response.txt WM_FODs.mif -shell 1000 -mask nodif_brain_mask.nii.gz -fslgrad " + bvecs + " " + bvals + " -force")
            os.system("sh2peaks WM_FODs.mif peaks.nii.gz -force")
