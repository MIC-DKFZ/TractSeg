    ...
    def generate_train_batch(self):
            ...
            try:
                if self.Config.FEATURES_FILENAME == "12g90g270g":
                    ...
                    if rnd_choice < 0.33:
                        ...
                        data = nib.load(join(C.DATA_PATH, self.Config.DATASET_FOLDER, subjects[subject_idx], "12g_125mm_peaks.nii.gz")).get_data()
                elif self.Config.FEATURES_FILENAME == "T1_Peaks270g":
                    peaks = nib.load(join(C.DATA_PATH, self.Config.DATASET_FOLDER, subjects[subject_idx], "270g_125mm_peaks.nii.gz")).get_data()
                    t1 = nib.load(join(C.DATA_PATH, self.Config.DATASET_FOLDER, subjects[subject_idx], "T1.nii.gz")).get_data()
                    data = np.concatenate((peaks, t1), axis=3)
                elif self.Config.FEATURES_FILENAME == "T1_Peaks12g90g270g":
                    rnd_choice = np.random.random()
                    if rnd_choice < 0.33:
                        peaks = nib.load(join(C.DATA_PATH, self.Config.DATASET_FOLDER, subjects[subject_idx], "270g_125mm_peaks.nii.gz")).get_data()
                    elif rnd_choice < 0.66:
                        peaks = nib.load(join(C.DATA_PATH, self.Config.DATASET_FOLDER, subjects[subject_idx], "90g_125mm_peaks.nii.gz")).get_data()
                    else:
                        peaks = nib.load(join(C.DATA_PATH, self.Config.DATASET_FOLDER, subjects[subject_idx], "12g_125mm_peaks.nii.gz")).get_data()
                    t1 = nib.load(join(C.DATA_PATH, self.Config.DATASET_FOLDER, subjects[subject_idx], "T1.nii.gz")).get_data()
                    data = np.concatenate((peaks, t1), axis=3)
                else:
                    data = nib.load(join(C.DATA_PATH, self.Config.DATASET_FOLDER, subjects[subject_idx], self.Config.FEATURES_FILENAME + ".nii.gz")).get_data()

                seg = nib.load(join(C.DATA_PATH, self.Config.DATASET_FOLDER, subjects[subject_idx], self.Config.LABELS_FILENAME + ".nii.gz")).get_data()
                break
            except IOError:
                exp_utils.print_and_save(self.Config, "\n\nWARNING: Could not load file. Trying again in 20s (Try number: " + str(i) + ").\n\n")
            exp_utils.print_and_save(self.Config, "Sleeping 20s")
            sleep(20)
        # exp_utils.print_and_save(self.Config, "Successfully loaded input.")

        data = np.nan_to_num(data)    # Needed otherwise not working
        seg = np.nan_to_num(seg)

        data = dataset_utils.scale_input_to_unet_shape(data, self.Config.DATASET, self.Config.RESOLUTION)    # (x, y, z, channels)
        ...
        if self.Config.LABELS_FILENAME not in ["bundle_peaks_11_808080", "bundle_peaks_20_808080", "bundle_peaks_808080",
            ...
            if self.Config.DATASET in ["HCP_2mm", "HCP_2.5mm", "HCP_32g"]:
                ...
                seg = dataset_utils.scale_input_to_unet_shape(seg, self.Config.DATASET, self.Config.RESOLUTION)  # (x, y, z, classes)

        slice_idxs = np.random.choice(data.shape[0], self.batch_size, False, None)

        # Randomly sample slice orientation
        if self.Config.TRAINING_SLICE_DIRECTION == "xyz":
