...
class Slicer:
    ...
    def create_files(Config, slice, train_subjects, validate_subjects, test_subjects):
        ...
        Slicer._create_slices_file(Config, validate_subjects, join(target_data_dir, "validate_slices"), slice)
        Slicer._create_slices_file(Config, test_subjects, join(target_data_dir, "test_slices"), slice)
        Slicer._create_slices_file(Config, train_subjects, join(target_data_dir, "train_slices"), slice)
    ...
    def create_prob_files(Config, bundle, train_subjects, validate_subjects, test_subjects):
        ...
        Slicer._create_prob_slices_file(Config, validate_subjects, join(target_data_dir, "validate_slices"), bundle)
        Slicer._create_prob_slices_file(Config, validate_subjects, join(target_data_dir, "test_slices"), bundle)
        Slicer._create_prob_slices_file(Config, test_subjects, join(target_data_dir, "train_slices"), bundle)

    @staticmethod
    def _create_prob_slices_file(Config, subjects, filename, bundle, shuffle=True):

        mask_dir = join(C.HOME, Config.DATASET_FOLDER)
